  ...
  (implicit writes: Writes[Request], reads: Reads[Response]): Future[Option[Response]] = {
    Ajax.post(
      ...
      data = s"""
        {
          "dynamicLinkInfo": {
            ...
            "link": "${Json.parse(Json.toJson(request).toString)("longUrl").toString.replaceAll("^\"|\"$", "")}"
          }
        }
      """,
      ...
      xhr.status match {
        ...
        case 200 =>
          val response = Map(
            "id" -> Json.parse(xhr.responseText)("shortLink").toString.replaceAll("^\"|\"$", ""),
            "longUrl" -> "",
            "imageUrl" -> ""
          )
          val ret = Try(Json.toJson(response).as[Response])
